...
def web_search_tools(settings: dict, tavily_api_key: str) -> list[dict]:
    ...
    async def search_web(query: str) -> str:
        ...
        # TavilyClient is synchronous: called bare it parks the whole event loop —
        # Telegram polling, live monitors and all — for the length of the search.
        raw     = await asyncio.to_thread(tavily_client.search, query=query, max_results=5)
