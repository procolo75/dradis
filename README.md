...
- **Token budget** — the free-tier ceiling that bites is *tokens per minute*, a rolling 60-second budget counting every call a turn makes, and a turn re-sends the whole conversation on each tool round. DRADIS stays inside it rather than discovering it from a rejection: a fixed sampling temperature so a prompt costs the same number of rounds every run, a bound on those rounds, tool results trimmed to the model's real headroom, requests paced against a per-provider token bucket, and one retry on the delay a rate limit asks for. The size estimate is measured rather than assumed — tool schemas counted, text priced at the worst observed ratio, and the provider's own usage figures correcting it after every round — and a request too large for a whole minute is trimmed before it is sent, because waiting cannot shrink one. The per-minute ceiling is a setting, so moving off the free tier is a number to change. Plus a `max_tokens` completion cap, per-round token logging, and an optional *Log token usage* toggle that appends `🔢 in N · out N` to every reply
...
- **Live Monitors** — persistent push-based monitors that react to external events in real time:
  ...
  - 🌧️ **Rain front** — reads the national weather radar (Dipartimento della Protezione Civile composite of 24 radars, Open Access, **no API key**) instead of a forecast model: where it is raining *now*, measured rather than predicted. Same bounded ring ladder as the storm front, with the field's drift and closest approach — and no drift at all below half a pixel per frame, which is the only honest answer at 1 km resolution and 5-minute cadence; the feed is a reference-counted singleton, so a second monitor watching another town costs nothing extra. No new dependencies — Pillow decodes the float32 GeoTIFF, numpy samples it. Each alert also carries **what the nearest rain gauge actually measured** (MeteoHub / Agenzia ItaliaMeteo, ~5000 ground stations, no API key) — naming the station, the rate and the distance. Diagnostic only: it is printed beside the radar's estimate and decides nothing, because a gauge publishes later than the radar and a gauge that has stopped transmitting reads the same as a dry one
  ...
  - ⚽ **Football Betting** — polls a live-odds API every 5 min; alerts when a losing team's next-goal odds beat the winning team's and are below that window's own maximum, inside two 2nd-half minute windows whose bounds and maxima are settable; provider fallback; timezone-aware quiet hours; no cron, no LLM
...
- Vertical left sidebar with collapsible sections: Settings (DRADIS, MQTT / Home Assistant, Position, MeteoHub, Car Mode, Telegram Bots), Tools (Web Search, Weather, Calendar, Gmail, Tasks, URL Fetch, Voice), Tasks, Scheduled Monitors, Live Monitors, HA Monitors
