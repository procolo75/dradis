...
- **Web Search** sub-agent: query search via Tavily (optional, free API key); **Web Reader** sub-agent: URL content fetch via Jina Reader (free, no API key required) — both activated by the single `ws_enabled` toggle
...
> → DRADIS routes to the dedicated **Web Reader** sub-agent, fetches the page via Jina Reader, and returns a concise summary. No API key required.
